...
static void onconnection(uv_stream_t *tcp, int status) {
  ...
  if (server->num_clients == MAX_CLIENTS) {
    printf("exceeded allowed number of clients");
...
static void alloc_cb(uv_handle_t *handle, size_t size, uv_buf_t *buf) {
  ...
  buf->len = size;
  if (buf->base == NULL) printf("alloc_cb buffer didn't properly initialize");
}
...
void luv_server_send(luv_server_t* self, luv_client_t* client, char* msg, int len)
{
  ...
  if (client == NULL) {
    printf("Client %d was not properly initialized, cannot send message to it.", client->id);
    return;
  }
...
void luv_server_start(luv_server_t* self, uv_loop_t *loop) {
  ...
  CHECK(r, "uv_listen");
  printf("Listening on %s:%d", self->host, self->port);
}
