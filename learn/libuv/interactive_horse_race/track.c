...
static void horse_draw(luv_horse_t *self) {
  if (!DRAW) {
    printf("Horse %s\ttrack: %d\t position: %d", self->name, self->track, self->position);
    return;
  }
  ...
  for (i = 0; i < HORSE_HEIGHT; i++) {
    mvprintw((self->track * HORSE_HEIGHT) + i, self->position, "%s", horse_pic[i]);
  }
  refresh();
}
...
static void update_player(int rand_num, luv_player_t* player) {
  ...
  horse->position++;
  printf("Horse %s progresses to position %d.", horse->name, horse->position);
  if (DRAW) horse_draw(player->horse);
}
