...
void draw_horse(horse_t *horse)
{
  if (!DRAW)
  {
    printf("Horse %s\ttrack: %d\t position: %d", horse->name, horse->track, horse->position);
    return;
  }
  ...
  {
    mvprintw((horse->track * HORSE_HEIGHT) + i, horse->position, "%s", horse_pic[i]);
  }
  refresh();
}
...
void finished_race_cb(uv_work_t *work_req, int status)
{
  ...
  horse_t *horse = work_req->data;
  if (!DRAW)
    printf("Horse %s finished race", horse->name);
  printf("Horse %s finished in place %d", horse->name, placement++);
...
void add_horse(uv_loop_t *loop, int track)
{
  ...
  if (!DRAW)
    printf("Queued horse %s on track: %d", horse->name, horse->track);
}
